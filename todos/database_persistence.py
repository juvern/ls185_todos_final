...
logger = logging.getLogger(__name__) # create an object
...
class DatabasePersistence:
    ...
    def find_list(self, list_id):
        ...
        lst.setdefault('todos', todos) # lst['todos'] = todos
        return lst
    ...
    def update_list_by_id(self, list_id, new_title):
        ...
        with self._database_connect() as conn:
            with conn.cursor() as cursor:
                cursor.execute(query, (new_title, list_id, )) # trailing comma not required but out of habit
    ...
    def update_todo_status(self, list_id, todo_id, new_status):
        query = """UPDATE todos SET completed = %s 
                ...
                """
        logger.info("Executing query: %s with list_id: %s, id: %s, status: %s", query, list_id, todo_id, new_status)
        with self._database_connect() as conn:
            with conn.cursor() as cursor:
